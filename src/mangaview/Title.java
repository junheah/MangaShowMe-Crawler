...
public class Title {
    ...
    public Title(String url) {
    	this.id = Integer.parseInt(url.split("manga_id=")[1].split("&")[0]);
    }
    ...
    public void fetchEps(CustomHttpClient client) {
        ...
        try {
            ...
            Elements epsE = items.select("div.slot");
            for(int i = 0; i<epsE.size(); i++) {
            	Element e = epsE.get(i);
                eps.add(new Manga(epsE.size()-i
                		,Integer.parseInt(e.attr("data-wrid"))
